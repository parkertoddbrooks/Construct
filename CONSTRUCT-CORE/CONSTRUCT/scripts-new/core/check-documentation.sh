...
main() {
    ...
    while IFS= read -r pattern; do
        ...
        local pattern_script="$SCRIPTS_ROOT/patterns/$pattern/validate-documentation.sh"
...
if [[ "$1" == "--help" || "$1" == "-h" ]]; then
    ...
    echo ""
    echo "CONSTRUCT Documentation Validation - Master Script"
    echo ""
    echo "This script orchestrates documentation validation across all active patterns."
    echo "It runs base checks and then delegates to pattern-specific validators."
    echo ""
    echo "Arguments:"
    echo "  PROJECT_DIR   Directory to check documentation in (default: current directory)"
    echo ""
    echo "Options:"
    echo "  -h, --help    Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0                    # Check documentation in current directory"
    echo "  $0 .                  # Check documentation in current directory"
    echo "  $0 Projects/MyApp/ios # Check documentation in specific project"
    echo ""
    echo "Pattern validators are located in:"
    echo "  $SCRIPTS_ROOT/patterns/*/validate-documentation.sh"
    exit 0
fi
