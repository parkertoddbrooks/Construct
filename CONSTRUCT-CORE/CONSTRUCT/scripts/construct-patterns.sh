...
case "$COMMAND" in
    ...
    "validate")
        ...
        # Check if file has generation metadata
        if ! grep -q "Hash:" "$PROJECT_DIR/CLAUDE.md" 2>/dev/null; then
            echo -e "${RED}❌ CLAUDE.md appears to be manually created (no generation hash found)${NC}"
            echo -e "${BLUE}💡 This file was not generated by the pattern system${NC}"
        ...
        if [ -f "$CONFIG_FILE" ] && command -v yq >/dev/null 2>&1; then
            ...
            echo -e "${YELLOW}⚠️ No patterns.yaml found or yq not installed${NC}"
            echo -e "${BLUE}💡 Cannot validate without pattern configuration${NC}"
            exit 0
        fi
        
        # Generate expected hash
        echo -e "${BLUE}🔄 Generating expected hash...${NC}"
        if [ -n "$LANGUAGES" ]; then
            EXPECTED_HASH=$("$ASSEMBLE_SCRIPT" "$PROJECT_DIR" "$PLUGINS" --languages "$LANGUAGES" --dry-run 2>/dev/null | tail -1)
        else
            EXPECTED_HASH=$("$ASSEMBLE_SCRIPT" "$PROJECT_DIR" "$PLUGINS" --dry-run 2>/dev/null | tail -1)
        fi
        
        if [ -z "$EXPECTED_HASH" ]; then
            echo -e "${RED}❌ Failed to generate expected hash${NC}"
            echo -e "${BLUE}💡 Check your patterns.yaml configuration${NC}"
            exit 1
        fi
        
        # Extract actual hash from file metadata
        ACTUAL_HASH=$(grep "Hash:" "$PROJECT_DIR/CLAUDE.md" 2>/dev/null | awk '{print $2}' || echo "")
        
        if [ -z "$ACTUAL_HASH" ]; then
            echo -e "${RED}❌ Could not extract hash from CLAUDE.md${NC}"
            echo -e "${BLUE}💡 File may be corrupted or manually edited${NC}"
            exit 1
        fi
        
        if [ "$EXPECTED_HASH" = "$ACTUAL_HASH" ]; then
            echo -e "${GREEN}✅ CLAUDE.md is unmodified${NC}"
            echo -e "${BLUE}📋 Hash: $ACTUAL_HASH${NC}"
        else
