...
check_existing_scripts() {
    ...
    local script_files=$(find "$CONSTUCT_DEV" -name "*.sh" -type f -not -path "*/lib/*")
...
check_configuration() {
    ...
    local config_files=$(find "$CONSTUCT_DEV/config" -name "*.yaml" -type f)
...
show_next_steps() {
    ...
    echo "  ./AI/scripts/check-architecture.sh    # Verify current state"
    echo "  # ... make your changes ..."
    echo "  ./AI/scripts/update-context.sh        # Update development context"
    echo "  ./AI/scripts/check-architecture.sh    # Verify changes"
    echo ""
