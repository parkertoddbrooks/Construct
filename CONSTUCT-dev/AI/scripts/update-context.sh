...
generate_auto_update_section() {
    ...
    local lib_functions=$(find "$CONSTUCT_DEV/lib" -name "*.sh" -type f | wc -l)
    local config_files=$(find "$CONSTUCT_DEV/config" -name "*.yaml" -type f | wc -l)
...
main() {
    ...
    echo ""
    echo "Next steps:"
    echo "  ./AI/scripts/check-architecture.sh   # Validate CONSTRUCT patterns"
    echo "  ./AI/scripts/before_coding.sh func   # Search before coding"
}
