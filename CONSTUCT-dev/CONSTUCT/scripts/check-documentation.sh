...
check_config_docs() {
    ...
    find "$CONSTUCT_DEV/CONSTUCT/config" -name "*.yaml" -type f | while read -r config_file; do
        ...
        local sections=$(grep -c "^[a-zA-Z_][a-zA-Z0-9_]*:" "$config_file")
        local comments=$(grep -c "^#.*:" "$config_file" || echo "0")
...
check_dev_docs() {
    ...
    for doc_file in "${dev_files[@]}"; do
        if [ -f "$CONSTUCT_DEV/$doc_file" ]; then
...
echo "  ./CONSTUCT/scripts/check-architecture.sh  # Validate architecture"
