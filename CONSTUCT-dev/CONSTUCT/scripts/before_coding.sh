...
source "$CONSTUCT_DEV/CONSTUCT/lib/file-analysis.sh"
...
check_existing_scripts() {
    ...
    local script_files=$(find "$CONSTUCT_DEV" -name "*.sh" -type f -not -path "*/CONSTUCT/lib/*")
...
check_configuration() {
    ...
    local config_files=$(find "$CONSTUCT_DEV/CONSTUCT/config" -name "*.yaml" -type f)
...
check_documentation() {
    ...
    local doc_files=$(find "$CONSTUCT_DEV" -name "*.md" -type f)
    local found_docs=0
...
check_templates() {
    ...
    local template_files=$(find "$CONSTUCT_DEV/Templates" -type f 2>/dev/null || true)
...
show_next_steps() {
    ...
    echo "  ./CONSTUCT/scripts/check-architecture.sh    # Verify current state"
    echo "  # ... make your changes ..."
    echo "  ./CONSTUCT/scripts/update-context.sh        # Update development context"
    echo "  ./CONSTUCT/scripts/check-architecture.sh    # Verify changes"
    echo ""
