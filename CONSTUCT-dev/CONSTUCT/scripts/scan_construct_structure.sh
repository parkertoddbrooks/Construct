...
if [ -d "$CONSTUCT_DEV/CONSTUCT/config" ]; then
...
lib_files=$(find "$CONSTUCT_DEV/CONSTUCT/lib" -name "*.sh" -type f | wc -l | tr -d ' ')
echo "Library Files: $lib_files" >> "$OUTPUT_FILE"

# Config completeness
config_files=$(find "$CONSTUCT_DEV/CONSTUCT/config" -name "*.yaml" -type f | wc -l | tr -d ' ')
...
if [ -f "$CONSTUCT_DEV/AI/scripts/check-architecture.sh" ]; then
    echo "- [ ] Run ./CONSTUCT/scripts/check-architecture.sh for current quality status" >> "$OUTPUT_FILE"
else
...
find "$CONSTUCT_DEV/CONSTUCT/lib" -name "*.sh" -type f | xargs -I {} basename {} | sort >> "$QUICK_REF"

echo "" >> "$QUICK_REF"
echo "## Configuration Files" >> "$QUICK_REF"
find "$CONSTUCT_DEV/CONSTUCT/config" -name "*.yaml" -type f | xargs -I {} basename {} | sort >> "$QUICK_REF"

echo -e "${BLUE}📄 Quick reference saved to: $QUICK_REF${NC}"
echo ""
echo "Next steps:"
echo "  ./CONSTUCT/scripts/update-context.sh      # Update development context"
echo "  ./CONSTUCT/scripts/check-architecture.sh  # Validate current architecture"