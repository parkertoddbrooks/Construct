...
source "$CONSTUCT_DEV/CONSTUCT/lib/template-utils.sh"
...
check_shell_script_quality() {
    ...
    local scripts=$(find "$CONSTUCT_DEV" -name "*.sh" -type f)
...
check_directory_structure() {
    ...
    local required_dirs=("AI" "CONSTUCT/lib" "CONSTUCT/config" "CONSTUCT/scripts")
    ...
    if [ ! -f "$CONSTUCT_DEV/CLAUDE.md" ]; then
    ...
    for lib_file in "${lib_files[@]}"; do
        if [ ! -f "$CONSTUCT_DEV/CONSTUCT/lib/$lib_file" ]; then
    ...
    for config_file in "${config_files[@]}"; do
        if [ ! -f "$CONSTUCT_DEV/CONSTUCT/config/$config_file" ]; then
...
check_code_duplication() {
    ...
    for pattern in "${common_patterns[@]}"; do
        local occurrences=$(find "$CONSTUCT_DEV" -name "*.sh" -exec grep -l "$pattern" {} \; 2>/dev/null | wc -l)
    ...
    local function_names=$(find "$CONSTUCT_DEV" -name "*.sh" -exec grep -h "^[a-zA-Z_][a-zA-Z0-9_]*(" {} \; 2>/dev/null | cut -d'(' -f1 | sort | uniq -d)
...
check_documentation() {
    ...
    for dir in "${dirs_needing_readme[@]}"; do
        if [ -d "$CONSTUCT_DEV/$dir" ] && [ ! -f "$CONSTUCT_DEV/$dir/README.md" ]; then
    ...
    local lib_files=$(find "$CONSTUCT_DEV/CONSTUCT/lib" -name "*.sh" -type f)
...
check_configuration_validity() {
    ...
    local yaml_files=$(find "$CONSTUCT_DEV/CONSTUCT/config" -name "*.yaml" -type f)
...
generate_summary() {
    ...
    echo ""
    echo "Next steps:"
    echo "  ./CONSTUCT/scripts/update-context.sh  # Update development context"
    echo "  ./CONSTUCT/scripts/before_coding.sh   # Check before adding features"
}
