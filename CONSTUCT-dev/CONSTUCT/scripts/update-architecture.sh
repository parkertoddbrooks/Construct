...
EOF
    ...
    find "$CONSTUCT_DEV/CONSTUCT/lib" -name "*.sh" -type f | sort | while read -r lib_file; do
...
EOF
    ...
    find "$CONSTUCT_DEV/CONSTUCT/config" -name "*.yaml" -type f | sort | while read -r config_file; do
...
load_validation_rules() {
    local config_file="\$CONSTUCT_DEV/CONSTUCT/config/quality-gates.yaml"
...
update_main_architecture() {
    ...
    local lib_count=$(find "$CONSTUCT_DEV/CONSTUCT/lib" -name "*.sh" -type f | wc -l | tr -d ' ')
    local config_count=$(find "$CONSTUCT_DEV/CONSTUCT/config" -name "*.yaml" -type f | wc -l | tr -d ' ')
...
EOF
    ...
    find "$CONSTUCT_DEV/CONSTUCT/lib" -name "*.sh" -type f | sort | while read -r lib_file; do
        ...
        grep -n "^[a-zA-Z_][a-zA-Z0-9_]*()" "$lib_file" | while IFS=: read -r line_num func_def; do
            ...
            echo "source \"\$CONSTUCT_DEV/CONSTUCT/lib/$lib_name.sh\"" >> "$output_file"
...
main() {
    ...
    echo "  ./CONSTUCT/scripts/check-quality.sh       # Validate documentation quality"
}
