...
# Function to scan for specific file patterns in CONSTRUCT-dev
scan_for_pattern() {
    local pattern=$1
    local title=$2
...
analyze_shell_scripts() {
    ...
    # Find all shell scripts in CONSTRUCT-dev
...
create_tree_structure() {
    ...
    if command -v tree &> /dev/null; then
        cd "$CONSTRUCT_ROOT"
        tree -I 'build|.git|node_modules|*.xcworkspace|*.xcodeproj|_old' CONSTRUCT-dev >> "$OUTPUT_FILE"
    else
